...
public:
    ...
    void setupTest(const std::string &name)
    {
        ...
        std::ignore = system(("./assemble.sh hazards_test_asm/" + name_ + ".s").c_str());
    ...
    void initSimulation()
    {
        ...
        top_->trace(tfp_, 99);
        std::string vcd_dir = "custom_test_out/" + name_;
        std::string cmd = "mkdir -p " + vcd_dir;
        std::ignore = system(cmd.c_str());
        tfp_->open(("custom_test_out/" + name_ + "/waveform.vcd").c_str());
    ...
    void TearDown() override
    {
        ...
        std::ignore = system(("mv data.hex custom_test_out/" + name_ + "/data.hex").c_str());
        std::ignore = system(("mv program.hex custom_test_out/" + name_ + "/program.hex").c_str());
    }
